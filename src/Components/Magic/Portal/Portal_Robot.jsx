...
export default function Portal_Robot({ nodes })
{
    ...
    const [posA] = useState(() => {
            ...
            nodes['Portal_Light004'].getWorldPosition(pos)
            return pos
    })

    const [posB] = useState(() => {
        const pos = new THREE.Vector3()
        nodes['Portal_Light007'].getWorldPosition(pos)
        return pos
    })
    ...
    return (
        <>
            {/* Portal A */}
            <mesh
                geometry={nodes['Portal_Light004'].geometry}
                position={nodes['Portal_Light004'].position}
                rotation={nodes['Portal_Light004'].rotation}
                frustumCulled={false}
                renderOrder={1}
            >
            ...
            {/* Portal B */}
            <mesh
                geometry={nodes['Portal_Light007'].geometry}
                position={nodes['Portal_Light007'].position}
                rotation={nodes['Portal_Light007'].rotation}
                frustumCulled={false}
                renderOrder={1}
            >
            ...
            {nearPortal && (
                <Html position={promptPos} center style={{ pointerEvents: 'none' }}>
                    <div style={{
                        ...
                        Press <strong>Enter</strong> to teleport (Robot)
                    </div>
                </Html>
            )}
        </>
