...
export default function Portal({ nodes })
{
    ...
    const [posA] = useState(() => {
        ...
        nodes['Portal_Light002'].getWorldPosition(pos)
        return pos
    })

    const [posB] = useState(() => {
        const pos = new THREE.Vector3()
        nodes['Portal_Light005'].getWorldPosition(pos)
        return pos
    })
    ...
    return (
        <>
            {/* Portal A */}
            <mesh
                geometry={nodes['Portal_Light002'].geometry}
                position={nodes['Portal_Light002'].position}
                rotation={nodes['Portal_Light002'].rotation}
                frustumCulled={false}
                renderOrder={1}
            >
            ...
            {/* Portal B */}
            <mesh
                geometry={nodes['Portal_Light005'].geometry}
                position={nodes['Portal_Light005'].position}
                rotation={nodes['Portal_Light005'].rotation}
                frustumCulled={false}
                renderOrder={1}
            >
            ...
            {nearPortal && (
                <Html position={promptPos} center style={{ pointerEvents: 'none' }}>
                    <div style={{
                        ...
                        Press <strong>Enter</strong> to teleport (Forest)
                    </div>
                </Html>
            )}
        </>
