...
export default function Resume() {
    ...
    const { scene: modelScene } = useGLTF('/Markers/Resume.glb', '/draco/')
    ...
    const { scale, position, rotation, proximityRange } = useControls('Marker', {
        Resume: folder({
            ...
            Particles: folder({ 
                pointSize: {
                    value: 0.03, min: 0, max: 2, step: 0.01,
                    onChange: (v) => { if (materialRef.current) materialRef.current.uniforms.uSize.value = v },
                },
                colorA: { value: '#10e826', onChange: (v) => materialRef.current?.uniforms.uColorA.value.set(v) },
    ...
    useEffect(() => {
        const onKey = (e) => {
            ...
            if (proximityState.current === 'near') {
                hidePrompt()
                window.dispatchEvent(new CustomEvent('openMarkerResume'))
            }
        }
...
useGLTF.preload('/Markers/Resume.glb')