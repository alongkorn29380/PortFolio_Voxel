...
export default function Old_Terrain(){
    const { size, viewport } = useThree()
    const materialRef = useRef()
    const indexRef = useRef(CUBE)

    const { scene: modelScene } = useGLTF('/Markers/Old_Terrain.glb', '/draco/')
    ...
    const { scale, position, rotation, proximityRange} = useControls('Marker', {
        Old_Terrain: folder ({
            Transform: folder({
                ...
                position: { value: [-40, 1, -33], step: 0.1},
            ...
            Particles: folder({
                ...
                colorA: { value: '#00cc2b', onChange: (v) => materialRef.current?.uniforms.uColorA.value.set(v) },
                colorB: { value: '#be0707', onChange: (v) => materialRef.current?.uniforms.uColorB.value.set(v) },
    ...
    useEffect(() => {
        const onKey = (e) => {
            ...
            if(proximityState.current === 'near') {
                hidePrompt()
                window.dispatchEvent(new CustomEvent('openMarkerOld_Terrain'))
            }
        }
...
useGLTF.preload('/Markers/Old_Terrain.glb')